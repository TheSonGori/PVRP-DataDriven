"""
Entorno Gymnasium del PVRP. Espacio de acciones discreto de tamaño N+1:
la acción 0 cierra la ruta actual y avanza de vehículo/día; las acciones
1..N visitan al cliente con ese índice de matriz. La asignación de patrones
de visita emerge implícitamente del orden de visitas, no es una decisión
explícita. Expone action_masks() para compatibilidad con MaskablePPO.

Entrada: una Instance (src/data/instance.py), un RewardConfig opcional y una
semilla; luego, acciones enteras en reset()/step().
Salida: tuplas Gymnasium (observación, recompensa, terminated, truncated,
info) y, vía get_solution(), la Solution construida durante el episodio.
"""
...
class PVRPEnv(gym.Env):
    ...
    # Suma, sobre todos los clientes, las visitas que quedaron sin realizar.
